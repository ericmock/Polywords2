...
struct stateStructure {
    var theta:[Float] = [Float](repeating:0.0, count: kNumParticles)
...
//
    ...
    func evaluate(with initial:stateStructure, deltaT dt:Float, d:derivative) -> derivative {
        var output:derivative = derivative()
        var accelXY:acceleration = acceleration()
